...
namespace SuRGeoNix.Flyleaf
{
    unsafe public class MediaDecoder
    {
        ...
        // Audio Output Parameters | [Default] Layout: Stereo, Channels: 2, Bits: 32 Float, Rate: Same as Input Rate | Prepared for user customization [based on ch_layout]
        AVSampleFormat _SAMPLE_FORMAT   = AVSampleFormat.AV_SAMPLE_FMT_FLT; int _CH_LAYOUT = AV_CH_LAYOUT_STEREO; int _CHANNELS;// = 2; 
        public int _RATE { get; private set; } // Will be set from Input Format
        public int _BITS { get; private set; } // Will be set from Input Format

        byte**  m_dst_data = null;
        int     m_max_dst_nb_samples = -1;
        int     m_dst_linesize = 0;

        // Video Output Parameters | [Default] Pixel Format: RGBA, Decoding Threads: 4, HWAccel: Enabled (NV12 | P010) | Fallback1 -> YUV420P to RGBA with pixel shader, Fallback2: sws_scale to RGBA low quality
        ...
        private int ProcessAudioFrame(AVFrame* frame)
        {
            ...
            try
            {
                ...
                if (m_max_dst_nb_samples == -1)
                {
                    ...
                    m_max_dst_nb_samples = (int)av_rescale_rnd(frame->nb_samples, _RATE, aCodecCtx->sample_rate, AVRounding.AV_ROUND_UP);
                    fixed(byte*** dst_data = &m_dst_data)
                    fixed(int *dst_linesize = &m_dst_linesize)
                    ret = av_samples_alloc_array_and_samples(dst_data, dst_linesize, _CHANNELS, m_max_dst_nb_samples, _SAMPLE_FORMAT, 0);

                }

                fixed (int* dst_linesize = &m_dst_linesize)
                {
                    dst_nb_samples = (int)av_rescale_rnd(swr_get_delay(swrCtx, aCodecCtx->sample_rate) + frame->nb_samples, _RATE, aCodecCtx->sample_rate, AVRounding.AV_ROUND_UP);

                    if (dst_nb_samples > m_max_dst_nb_samples)
                    {
                        av_freep(&m_dst_data[0]);
                        ret = av_samples_alloc(m_dst_data, dst_linesize, _CHANNELS, (int)dst_nb_samples, _SAMPLE_FORMAT, 0);
                    }

                    ret = swr_convert(swrCtx, m_dst_data, dst_nb_samples, (byte**)&frame->data, frame->nb_samples);
                    if (ret < 0) return ret;

                    int dst_data_len = av_samples_get_buffer_size(dst_linesize, _CHANNELS, ret, _SAMPLE_FORMAT, 1);
        ...
        private int SetupAudio(AVFormatContext* fmtCtx = null)
        {
            ...
            _BITS                                   = aCodecCtx->bits_per_coded_sample;
            _CHANNELS                               = av_get_channel_layout_nb_channels((ulong)_CH_LAYOUT);
            ...
            av_opt_set_int(swrCtx,           "out_channel_layout",       _CH_LAYOUT, 0);
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        #region Decoder Context (To be seperated)
        [HandleProcessCorruptedStateExceptions]
        [SecurityCritical]
        ...
        {
            Log($"[{BytePtrToStringUTF8(decCtx.fmtCtx->iformat->long_name)} | {BytePtrToStringUTF8(decCtx.fmtCtx->iformat->name)} | {BytePtrToStringUTF8(decCtx.fmtCtx->iformat->mime_type)} | {BytePtrToStringUTF8(decCtx.fmtCtx->iformat->extensions)}]");
